...
import common.model.{SessionAggrStat, SessionRandomExtract, UserInfo, UserVisitAction}
...
object SessionStatisticAgg {
  ...
  def main(args: Array[String]): Unit = {
    ...
    session2FilteredRDD.cache()


    //日期小时全部session信息
    val dateHour2FullInfoRDD = session2FilteredRDD.map {
      case (sid, fullInfo) =>
    ...
    //小时session计数 格式：YYYY-MM-DD_HH:mm
    ...
    //一共多少天
    val days = dateHourCountMap.size
    //一天抽取多少条 100 / days
    val extractPerDay = 100 / days
    //一天有多少条session
    ...
    def generateRandomIndexList(extractPerDay: Long,
      ...
      for ((hour, count) <- hourCountMap) {
        ...
        var hourExrCout = ((count / daySessionCount.toDouble) * extractPerDay).toInt

        if (hourExrCout > count) {
          hourExrCout = count.toInt
        }

        val random = new Random()
        hourListMap.get(hour) match {
          case None => hourListMap(hour) = new ListBuffer[Int]
            for (i <- 0 until hourExrCout) {
